...
RUN for i in 1 2 3; do apt-get update && apt-get install --allow-unauthenticated -q -y gnupg && apt-get clean && break || if [ $i -lt 3 ]; then sleep 5; else false; fi; done
@[end if]@
...
RUN for i in 1 2 3; do apt-get update && apt-get install --allow-unauthenticated -q -y dpkg && apt-get clean && break || if [ $i -lt 3 ]; then sleep 5; else false; fi; done
@[end if]@
