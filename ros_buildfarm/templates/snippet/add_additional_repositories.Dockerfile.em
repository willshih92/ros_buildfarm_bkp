...
RUN for i in 1 2 3; do apt-get update -o Acquire::AllowInsecureRepositories=true && apt-get install -q -y apt-transport-https && apt-get clean && break || if [ $i -lt 3 ]; then sleep 5; else false; fi; done
